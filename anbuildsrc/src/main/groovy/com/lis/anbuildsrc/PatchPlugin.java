...
public class PatchPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        project.getExtensions().create("path", PatchExtension.class);
    ...
    private void configTasks(final Project project, final ApplicationVariant variant, final PatchExtension patchExtension) {
        //获得：debug/release
        ...
        dexTask.doFirst(new Action<Task>() {
            ...
            public void execute(Task task) {
                ...
                //用户配置的application,实际上可以解析manifest自动获取，但是java实现 太麻烦，让用户自己配置
                String applicationName = patchExtension.applicationName;
                //windows下目录输出是 xx\xx,linux下是、比、xx/xx，把.替换成平台相关的斜杠
                applicationName = applicationName.replaceAll("\\.", Matcher.quoteReplacement(File.separator));
                //记录类的md5
                Map<String, String> newHexs = new HashMap<>();
                //任务的输入，dex打包任务要输入什么？自然是所有的class和jar包
                final Set<File> files = dexTask.getInputs().getFiles().getFiles();
                for (File file : files) {
     ...
     */
    private void processClass(String applicationName, String dirName, File file, Map<String, String> hexs, PatchGenerator patchGenerator) {
        String filePath = file.getAbsolutePath();
    ...
    private void processJar(String applicationName, File file, Map<String, String> hexs, PatchGenerator patchGenerator) {
        try {
